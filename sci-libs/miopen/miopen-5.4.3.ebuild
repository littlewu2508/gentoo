...
if [[ ${PV} == *9999 ]] ; then
	inherit git-r3
	EGIT_REPO_URI="https://github.com/ROCmSoftwarePlatform/MIOpen.git"
	S="${WORKDIR}/${P}"
else
...
PATCHES=(
	...
	"${FILESDIR}/${PN}-5.4.3-enable-test.patch"
	...
	"${FILESDIR}/${PN}-5.4.3-link-llvm.patch"
)

src_prepare() {
	cmake_src_prepare
	sed -e "s,@LLVM_VERSION@,${LLVM_MAX_SLOT},g" -i CMakeLists.txt || die
...
src_compile() {
	# gtest_discover_tests needs run test binary at build time, which needs
	# find the libMIOpen.so in ${BUILD_DIR}/lib
	LD_LIBRARY_PATH="${BUILD_DIR}/lib" cmake_src_compile
}
