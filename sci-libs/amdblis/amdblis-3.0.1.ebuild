...
SRC_URI="https://github.com/amd/blis/archive/refs/tags/${PV}.tar.gz -> ${P}.tar.gz"
...
PATCHES=(
	"${FILESDIR}"/blis-0.6.0-rpath.patch
...
src_prepare() {
	...
	find -name Makefile -print0 | \
		while IFS= read -d '' -r fn; do
			sed -e 's,/blis,/amdblis,g' -i ${fn} || die
		done
}
...
src_install() {
	...
	if use eselect-ldso; then
		dodir /usr/$(get_libdir)/blas/amdblis
		insinto /usr/$(get_libdir)/blas/amdblis
		doins lib/*/lib{c,}blas.so.3
		dosym libblas.so.3 usr/$(get_libdir)/blas/amdblis/libblas.so
		dosym libcblas.so.3 usr/$(get_libdir)/blas/amdblis/libcblas.so
	fi
}
...
pkg_postinst() {
	...
	local libdir=$(get_libdir) me="amdblis"
