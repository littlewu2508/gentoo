...
src_prepare() {
	...
	eapply "${FILESDIR}/${P}-SWDEV-355608-Remove-clang-include-path-2996.patch"
	eapply "${FILESDIR}/${P}-SWDEV-344620-hipcc-fails-to-parse-version-of-clang-i.patch"
	...
	mv bin/hipvars.pm "${T}"/
	if [ ${PV} == 9999 ]; then
		cp "$(prefixify_ro "${FILESDIR}"/hipvars-9999.pm)" bin/hipvars.pm || die "failed to replace hipvars.pm"
		sed -n '/^\$HIP_BASE_VERSION_.* = "/p' "${T}"/hipvars.pm >> bin/hipvars.pm || die
		echo '$HIP_VERSION="$HIP_BASE_VERSION_MAJOR.$HIP_BASE_VERSION_MINOR.$HIP_BASE_VERSION_PATCH";' >> bin/hipvars.pm
	else
