...
PATCHES=(
	...
	"${FILESDIR}/${PN}-5.2.0-fix-install-destination-typo.patch"
)
...
src_prepare() {
	...
	sed -e "/LIB_INSTALL_DIR/s:PREFIX}/lib:PREFIX}/$(get_libdir):" \
		-e "/${HIP_COMMON_DIR}\/cmake DESTINATION/s: .): $(get_libdir)):" \
		-i CMakeLists.txt || die
		# -e "/\.hip/d" -i CMakeLists.txt || die
	...
	sed -e "s,DESTINATION lib,DESTINATION $(get_libdir),g" -i src/hiprtc/CMakeLists.txt || die

	pushd ${HIP_S} || die
	eapply "${FILESDIR}/${PN}-5.1.3-clang-include-path.patch"
	eapply "${FILESDIR}/${PN}-5.1.3-rocm-path.patch"
	eapply "${FILESDIR}/${PN}-5.2.0-correct-ldflag.patch"
